...
@NoArgsConstructor
@AllArgsConstructor
@Schema(description = "Scope 1 배출량 응답 DTO (프론트엔드와 1:1 매핑)")
public class Scope1EmissionResponse {
    @Schema(description = "Scope type", example = "scope 1 ")
    private String ScopeType;

    @Schema(description = "Scope 1 배출량 고유 식별자", example = "1")
    private Long id; // Scope 1 배출량 고유 식별자

    @Schema(description = "Scope 1 제품명", example = "엔진")
    private String companyProduct;

    @Schema(description = "Scope 1 제품 코드", example = "L11AC")
     ...
     */
    public static Scope1EmissionResponse from(ScopeEmission entity) {
        return Scope1EmissionResponse.builder()
                .id(entity.getId())
                .companyProduct(entity.getCompanyProduct()) //제품명 추가
                .companyProductCode(entity.getCompanyProductCode()) // 제품 코드 추가
                .majorCategory(entity.getMajorCategory()) // 고정,이동 등 구분 용도 (없어도 됨)
                ...
                .scope1CategoryNumber(entity.getScope3CategoryNumber()) // 스코프 1 카테고리 번호
                .scope1CategoryName(entity.getScope3CategoryName()) // 스코프 1 카테고리 이름
