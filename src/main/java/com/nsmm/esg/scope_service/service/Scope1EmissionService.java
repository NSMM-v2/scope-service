...
import com.nsmm.esg.scope_service.repository.Scope1EmissionRepository;
...
public class Scope1EmissionService {

    private final Scope1EmissionRepository scope1EmissionRepository;
     ...
     */
    @Transactional
    public Scope1EmissionResponse createScope1Emission(
            Scope1EmissionRequest request,
            String userType,
            String headquartersId,
            String partnerId,
            String treePath) {

        log.info("Scope 3 배출량 데이터 생성 시작: userType={}, categoryNumber={}", userType, request.getScope1CategoryNumber());
        ...
        log.debug("  - categoryNumber: {}", request.getScope1CategoryNumber());
        ...
        try {
            ...
            // 4. 카테고리 정보 조회
            log.debug("카테고리 정보 조회 시작: scope1CategoryNumber={}", request.getScope1CategoryNumber());
            Scope1Category category = Scope1Category.fromCategoryNumber(request.getScope1CategoryNumber());
            log.debug("카테고리 정보 조회 완료: scope1CategoryName='{}'", category.getScope1CategoryName());
            ...
            ScopeEmission savedEmission = scope1EmissionRepository.save(emission);
            log.info("Scope 3 배출량 데이터 생성 완료: id={}, totalEmission={}",
                    savedEmission.getId(), savedEmission.getTotalEmission());

            return Scope1EmissionResponse.from(savedEmission);
     ...
     * @return 배출량 데이터 목록
     */
    public Page<Scope1EmissionResponse> getScope1EmissionsByTreePath(String treePath, Pageable pageable) {
        log.info("TreePath 기반 배출량 데이터 조회: treePath={}", treePath);

        Page<ScopeEmission> emissions = scope1EmissionRepository.findByTreePathStartingWith(treePath, pageable);

        return emissions.map(Scope1EmissionResponse::from);
    }

    /**
     * 회사별 배출량 데이터 조회
     */
    public List<Scope1EmissionResponse> getScope1EmissionsByCompany(
            String userType,
            String headquartersId,
            String partnerId,
            String treePath) {
        ...
        if ("HEADQUARTERS".equals(userType)) {
            ...
            emissions = scope1EmissionRepository.findByHeadquartersId(Long.parseLong(headquartersId));
        } else if ("PARTNER".equals(userType)) {
            if (partnerId == null) {
                throw new IllegalArgumentException("협력사 ID가 필요합니다");
            }
            ...
            emissions = scope1EmissionRepository.findByPartnerIdAndTreePathStartingWith(
        ...
        return emissions.stream()
                .map(Scope1EmissionResponse::from)
     ...
     * 연도별 배출량 데이터 조회
     */
    public List<Scope1EmissionResponse> getScope1EmissionsByYear(
            Integer year,
            String userType,
            String headquartersId,
            String partnerId,
        ...
        if ("HEADQUARTERS".equals(userType)) {
            ...
            // 본사: 모든 하위 조직 데이터 조회 가능
            emissions = scope1EmissionRepository.findByHeadquartersIdAndReportingYear(
            ...
            // 협력사: 본인 및 하위 조직 데이터만 조회 가능
            emissions = scope1EmissionRepository.findByPartnerIdAndTreePathStartingWithAndReportingYear(
     ...
     * 카테고리별 배출량 데이터 조회
     */
    public List<Scope1EmissionResponse> getScope1EmissionsByCategory(
        ...
        if ("HEADQUARTERS".equals(userType)) {
            ...
            // 본사: 모든 하위 조직 데이터 조회 가능
            emissions = scope1EmissionRepository.findByHeadquartersIdAndScope1CategoryNumber(
            ...
            // 협력사: 본인 및 하위 조직 데이터만 조회 가능
            emissions = scope1EmissionRepository.findByPartnerIdAndTreePathStartingWithAndScope1CategoryNumber(
     ...
     * 연도/월별 배출량 데이터 조회
     */
    public List<Scope1EmissionResponse> getScope1EmissionsByYearAndMonth(
            Integer year,
            Integer month,
            String userType,
            String headquartersId,
        ...
        if ("HEADQUARTERS".equals(userType)) {
            ...
            // 본사: 모든 하위 조직 데이터 조회 가능
            emissions = scope1EmissionRepository.findByHeadquartersIdAndReportingYearAndReportingMonth(
            ...
            // 협력사: 본인 및 하위 조직 데이터만 조회 가능
            emissions = scope1EmissionRepository.findByPartnerIdAndTreePathStartingWithAndReportingYearAndReportingMonth(
     ...
     */
    public List<Scope1EmissionResponse> getScope1EmissionsByYearAndMonthAndCategory(
            Integer year,
            Integer month,
            Integer categoryNumber,
            String userType,
        ...
        if ("HEADQUARTERS".equals(userType)) {
            ...
            // 본사: 모든 하위 조직 데이터 조회 가능
            emissions = scope1EmissionRepository.findByHeadquartersIdAndReportingYearAndReportingMonthAndScope1CategoryNumber(
            ...
            // 협력사: 본인 및 하위 조직 데이터만 조회 가능
            emissions = scope1EmissionRepository
                    .findByPartnerIdAndTreePathStartingWithAndReportingYearAndReportingMonthAndScope1CategoryNumber(
    ...
    public Map<Integer, BigDecimal> getCategorySummaryByYearAndMonth(
        ...
        if ("HEADQUARTERS".equals(userType)) {
            ...
            // 본사: 모든 하위 조직 데이터 조회 가능
            if (month != null) {
                results = scope1EmissionRepository.sumTotalEmissionByCategoryAndYearAndMonthForHeadquarters(
                        Long.parseLong(headquartersId), year, month);
            } else {
                results = scope1EmissionRepository.sumTotalEmissionByCategoryAndYearForHeadquarters(
                        Long.parseLong(headquartersId), year);
            }
        } else if ("PARTNER".equals(userType)) {
            if (partnerId == null) {
            ...
            // 협력사: 본인 및 하위 조직 데이터만 조회 가능
            if (month != null) {
                results = scope1EmissionRepository.sumTotalEmissionByCategoryAndYearAndMonthForPartner(
                        Long.parseLong(partnerId), treePath, year, month);
            } else {
                results = scope1EmissionRepository.sumTotalEmissionByCategoryAndYearForPartner(
                        Long.parseLong(partnerId), treePath, year);
            }
        } else {
            throw new IllegalArgumentException("알 수 없는 사용자 유형입니다: " + userType);
     ...
     * @return 배출량 데이터
     */
    public Scope1EmissionResponse getScope1EmissionById(Long id, String userId, String userType, String treePath) {
        log.info("배출량 데이터 조회: id={}, userId={}, userType={}, treePath={}", id, userId, userType, treePath);

        ScopeEmission emission = scope1EmissionRepository.findById(id)
        ...
        return Scope1EmissionResponse.from(emission);
    }
     ...
     */
    @Transactional
    public Scope1EmissionResponse updateScope1Emission(
            Long id,
            Scope1EmissionUpdateRequest request,
            String userType,
            String headquartersId,
            String partnerId,
            String treePath) {
        ...
        // 2. 기존 데이터 조회
        ScopeEmission existingEmission = scope1EmissionRepository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("배출량 데이터를 찾을 수 없습니다: " + id));
        ...
        if (request.hasKeyFields()) {
            ...
            Scope1EmissionRequest tempRequest = createValidationRequest(request, existingEmission);
        ...
        if (request.getScope1CategoryNumber() != null) {
            builder.scope1CategoryNumber(request.getScope1CategoryNumber());
        }
        if (request.getScope1CategoryName() != null) {
            builder.scope1CategoryName(request.getScope1CategoryName());
        }
        ...
        // 7. 저장 및 응답
        ScopeEmission updatedEmission = scope1EmissionRepository.save(builder.build());
        log.info("Scope 3 배출량 데이터 업데이트 완료: id={}, totalEmission={}",
                updatedEmission.getId(), updatedEmission.getTotalEmission());

        return Scope1EmissionResponse.from(updatedEmission);
    }
     ...
     */
    @Transactional
    public void deleteScope1Emission(
            Long id,
            String userType,
            String headquartersId,
            String partnerId,
        ...
        // 3. 삭제
        scope1EmissionRepository.delete(emission);
    ...
    private void validateDuplicateData(
        ...
        if ("HEADQUARTERS".equals(userType)) {
            existingData = scope1EmissionRepository
                    .findByHeadquartersIdAndReportingYearAndReportingMonthAndCompanyProductCodeAndScope1CategoryNumberAndMajorCategoryAndSubcategoryAndRawMaterial(
                            headquartersId,
                            request.getReportingYear(),
                            request.getReportingMonth(),
                            request.getCompanyProductCode(),
                            request.getScope1CategoryNumber(),
                            request.getMajorCategory(),
                            request.getSubcategory(),
                            request.getRawMaterial());
        } else {
            existingData = scope1EmissionRepository
                    .findByPartnerIdAndReportingYearAndReportingMonthAndCompanyProductCodeAndScope1CategoryNumberAndMajorCategoryAndSubcategoryAndRawMaterial(
                            partnerId,
                            request.getReportingYear(),
                            request.getReportingMonth(),
                            request.getCompanyProductCode(),
     ...
     */
    private Scope1EmissionRequest createValidationRequest(
            Scope1EmissionUpdateRequest updateRequest,
            ScopeEmission existingEmission) {

        return Scope1EmissionRequest.builder()
                ...
                .scope1CategoryNumber(updateRequest.getScope1CategoryNumber() != null ? updateRequest.getScope1CategoryNumber()
                        : existingEmission.getScope1CategoryNumber())
